...
RUN echo -e "travis_fold:start:apt.packages\033[33;1mInstalling apt packages\033[0m" \
 && apt update \
 && apt install -y wget unzip \
 && echo -e "\ntravis_fold:end:apt.packages\r"
...
# Execute lint task
RUN echo -e "travis_fold:start:lint\033[33;1mRunning lint task\033[0m" \
 && ./gradlew --no-daemon lint \
 && echo -e "\ntravis_fold:end:lint\r"

# Execute checkstyle task
RUN echo -e "travis_fold:start:checkstyle\033[33;1mRunning check task\033[0m" \
 && ./gradlew --no-daemon checkstyleAndroid \
 && echo -e "\ntravis_fold:end:checkstyle\r"

# Execute build task
RUN echo -e "travis_fold:start:build\033[33;1mRunning build task\033[0m" \
